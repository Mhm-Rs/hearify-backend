...
public class UserController {
  ...
  public ResponseEntity<UserDto> authenticateUser(
      @Validated @RequestBody UserAuthenticationDto userAuthenticationDto) {
    return ResponseEntity.ok(
        userMapper.toUserDto(
            userService.authenticateUser(
                userMapper.fromUserAuthenticationDto(userAuthenticationDto))));
  }
  ...
  public ResponseEntity<List<FavoriteDto>> getUserFavorites(@PathVariable Integer id) {
    return ResponseEntity.ok(
        favoriteMapper.toFavoriteDtoList(favoriteService.getUserFavorites(id)));
  }
  ...
  @ApiResponse(responseCode = "500", description = HearifyConstants.INTERNAL_SERVER_ERROR)
  public ResponseEntity<Void> addTrackToFavorites(
      @PathVariable Integer userId, @RequestBody TrackManipulationDto trackManipulationDto) {
  ...
  @ApiResponse(responseCode = "500", description = HearifyConstants.INTERNAL_SERVER_ERROR)
  public ResponseEntity<Void> removeTrackFromFavorites(
      @PathVariable Integer userId, @PathVariable Integer trackId) {
